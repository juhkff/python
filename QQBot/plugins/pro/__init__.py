...
from plugins.pro.data_source import change_pro, search_name_in_settings


@on_command('pro', only_to_me=False, permission=SUPERUSER)
async def pro_change(session: CommandSession):
    is_format = session.get('format')
    if not is_format:
        await session.send('./pro 格式错误！./pro QQ 已有属性名 ±5')
        return
    # 格式正确
    ...
    try:
        cur_pro_content = change_pro(change_qq, change_number, pro_name, pro_index)
    except Exception:
    ...
    try:
        ...
        card = change_qq
    await session.send('[' + card + ']' + ' 目前的' + pro_name + '为： ' + cur_pro_content)
    return


# pro_change.args_parser装饰器将函数声明为hp命令的参数解析器
# 命令解析器用于将用户输入的参数解析成命令真正需要的数据
@pro_change.args_parser

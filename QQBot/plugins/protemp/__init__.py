...
from plugins.pro import search_name_in_settings
from plugins.protemp.data_source import temp_change_pro


@on_command('tpro', only_to_me=False, permission=SUPERUSER)
async def pro_change(session: CommandSession):
    is_format = session.get('format')
    if not is_format:
        await session.send('./tpro 格式错误！./tpro QQ 已有属性名 ±5')
        return
    # 格式正确
    ...
    try:
        cur_pro_content = temp_change_pro(change_qq, change_number, pro_name, pro_index)
    except Exception:

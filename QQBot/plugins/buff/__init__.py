...
async def buff(session: CommandSession):
    ...
    # 存在状态卡
    try:
        result_str = update_buff(change_qq, op_type, buff_name)
    except Exception:
    ...
    if result_str == 'Not have':
        await session.send('[' + card + ']' + buff_remove_not_have)
    if result_str == 'Succeed':
        await session.send('[' + card + ']' + buff_update_succeed)
    return


# buff.args_parser装饰器将函数声明为buff命令的参数解析器
...
async def _(session: CommandSession):
    ...
    if len(args) != 3:
        ...
        session.state['buff_name'] = buff_name
        return
